...
### 30.4 训练结果（seed0，ep1–20 已出，ep21–24 进行中）
训练正常，无 NaN/梯度爆炸。ep12–16 判定窗口已完整。
...
基线 = clean full RSSM（run10 seed0，38.39）：| 指标 | 基线 | 本次 | Δ | 标准 | 判定 |
|---|---:|---:|---:|---|:---:|
...
### 30.6 核心结论（rotation-only 不通过）
1. **Pedestrian strict 无提升**：ep12–16 mean 0.161 vs 基线 0.22，且 0.10–0.27 全程噪声带内
   震荡（ep14 的 0.269 是单点，未能复现）。yaw 初始误差 45°→22.5° 没有带来 strict 的实质
   改善，与 A/B/C/D 表诊断中「strict 被 w/yaw 误差上限卡死、而非 anchor 匹配角分辨率」一致。
2. **Cyclist loose 被拖低 −4.56**：rotation-only 并未真正定位到 Ped（`rotations` 被
   `Anchor3DRangeGenerator` 跨 6 个 size 共享），Cyc/Car/Truck 也各多出 45°/135° 朝向，
   Cyc 首当其冲受伤（重复第 26–29 节 anchor 层变更伤 Cyc 的模式）。
3. **Truck strict +1.93 / Ped loose +0.73 是唯一正向**，但无法抵消 Overall −0.64；
   且 Car strict 方差极大（39.65–51.90），说明 anchor 信道翻倍引入了明显不稳定。
