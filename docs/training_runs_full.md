...
- **Cyclist 两个 seed 都无增益**（-0.72 / -0.01），仅保住 -1.0 红线，说明该分支并没能
  在 Cyclist 上带来收益，做的是刻意的 Car/Cyclist 类别资源再分配。
...
2. 若三 seed 均值也无法支持该分支，则 CycCls 分支按「Car 单类特化」归档，主模型维持
   Run 10 head-v2 `40.42 ± 0.51`，不再为整体 Overall 调整该分支。
