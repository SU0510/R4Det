...
### 47.5 `fgfull_N4_temporal_baseline` 实际启动（无 val 结果）
...
- 该 run 是 46.12 中定义的控制组：`no2d_igdr` stack + `TemporalDeformableFusionBaseline`，
  ...
  也没有 IGDR。
- 截至 2026-09-22 10:56 UTC 的日志检查，训练已进入 ep1 并持续输出 train iter，但还没有
  写入任何 `mode=val` 行，因此本节不给出训练结果，也不能提前判定 temporal baseline
