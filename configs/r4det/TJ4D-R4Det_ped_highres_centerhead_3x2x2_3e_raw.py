...
# default settings
freeze_images = True                         # frozen (Ped isolation stage 1)
freeze_depths = True
freeze_radars = True
use_grid_mask = False
camera_stream = 'LSS'
...
model = dict(
    ...
    ped_highres_stage=True,
    highres_ped_branch=dict(
        type='PedHighresBranch',
        radar_channels=64,
        fused_channels=_dim_,
        out_channels=_dim_,
        channels=64,
        num_convs=2,
        kernel_size=3),
    ...
    # temporal fusion with Motion-Aligned RSSM (deformable state alignment)
    temporal_fusion=dict(
        type='MotionAlignedRSSMFusion',
        in_channels=_dim_,
        out_channels=_dim_,
        kernel_size=3,
        latent_dim=_dim_,
        hidden_dim=128,
        action_dim=0,  # velocity branch disabled (TJ4D has no ego pose); set >0 + pass velocity to re-enable
        kl_scale=1.0,
        free_nats=1.0,
        min_std=0.1,
        init_std=0.2,
    ...
    seq_len=4,  # temporal sequence length (current + 3 history frames) for RSSM
    # Independent Pedestrian CenterHead (stage-1 isolation). Sibling of the
    # frozen Anchor3DHead below so the clean checkpoint loads the anchor head
    # (unchanged keys) and only this head gets freshly-initialized weights.
    ped_center_head=dict(
        ...
        bbox_coder=dict(
            ...
            score_threshold=0.0,
            out_size_factor=1,
            voxel_size=[voxel_size[0] / 2, voxel_size[1] / 2],
            code_size=7),
        train_cfg=dict(
            grid_size=[bev_w_ * 2, bev_h_ * 2, 1],
            voxel_size=[voxel_size[0] / 2, voxel_size[1] / 2],
            point_cloud_range=point_cloud_range,
            out_size_factor=1,
            dense_reg=1,
...
log_level = 'INFO'
# Stage1 Ped head provides the detection-head initialization. The high-res
# branch is intentionally new and starts from random initialization.
