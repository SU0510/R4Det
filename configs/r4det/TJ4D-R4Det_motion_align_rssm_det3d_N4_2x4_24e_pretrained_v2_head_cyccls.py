...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        # Cyclist-only classification residual branch: adds a learned residual
        # to the Cyclist logit channel of every anchor (12 anchors -> 12 Cyclist
        # logits). Final 1x1 is zero-init so training starts from the shared stem
        # baseline. Other classes / regression / RSSM are untouched.
        cyc_cls_branch=True,
        cyc_cls_branch_channels=64,
        cyc_cls_branch_class=1,   # class_names = [Pedestrian, Cyclist, Car, Truck]
