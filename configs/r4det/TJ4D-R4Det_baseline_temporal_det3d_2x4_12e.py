...
model = dict(
    ...
    # temporal fusion with TemporalDeformableFusion (original baseline)
    temporal_fusion=dict(
        type='TemporalDeformableFusion',
        in_channels=_dim_,
        out_channels=_dim_,
        kernel_size=3,
        deform_groups=1,
...
test_pipeline = [
    ...
    dict(type='CustomCollect3D2', keys=['points', 'img'],
         meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                    'cam2img', 'pad_shape',
                    'scale_factor', 'flip', 'pcd_horizontal_flip',
                    'pcd_vertical_flip', 'box_mode_3d', 'box_type_3d',
