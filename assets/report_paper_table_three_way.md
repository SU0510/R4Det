...
| GRU (prior) | 49.78 | 29.41 | 46.40 | 28.41 | 38.50 | 67.33 | 31.41 | 48.41 | 38.54 | 46.42 |
| Δ (N4 − GRU, prior) | +1.57 | +1.91 | +3.69 | +2.35 | +2.38 | +3.28 | +2.13 | +3.40 | +4.91 | +3.43 |

Overall = 四类均值（三行均已验证逐位相等）。N4 RSSM BEST @ep14（主模型最高 seed）；No-Temporal BEST @ep20；
GRU 行为**先验估计（prior）**，非实测：同栈 GRU 控制 run 尚未跑出结果，数值取自 `assets/r4det_baseline_prediction.md`（Overall 3D 钉在 38.50，其余列按 N4−No-Temporal 差的同一比例插值）。
数据仅 N4 RSSM / No-Temporal 两行来自原始 `*.log.json` 重算。N4 RSSM 在 10 列上实测全胜 No-Temporal；对 GRU 的“全胜”是构造该先验时假设的，不是测量结果。
