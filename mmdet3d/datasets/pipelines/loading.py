...
class LoadOccGTFromFile(object):
    ...
    def __call__(self, results):
        ...
        if 'hop_load_all' in results and results['hop_load_all']:
            num_frames = len(results['hop_all_path']['adj_path'])
    ...
    def __call__(self, results):
        ...
        if self.with_label_3d:
            ...
            if 'gt_labels_3d' in results:
                invalid_label_mask = results['gt_labels_3d'] == -1
                if invalid_label_mask.any():
                    num_invalid = torch.sum(torch.from_numpy(invalid_label_mask)).item()
                    valid_indices = ~invalid_label_mask
                    results['gt_labels_3d'] = results['gt_labels_3d'][valid_indices]
                    if 'gt_bboxes_3d' in results:
                        results['gt_bboxes_3d'] = results['gt_bboxes_3d'][valid_indices]
                    if 'gt_bboxes' in results:
                        results['gt_bboxes'] = results['gt_bboxes'][valid_indices]
                    if 'gt_labels' in results:
                        results['gt_labels'] = results['gt_labels'][valid_indices]
