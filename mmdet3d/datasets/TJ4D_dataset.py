...
class TJ4DDataset(Custom3DDataset):
    ...
    def _create_empty_frame_data(self, data_curr):
        ...
        for key, val in data_curr.items():
            ...
            if key == 'points':
                empty_data = torch.zeros((0, inner.shape[1]), dtype=inner.dtype) if isinstance(inner,
                                                                                               torch.Tensor) else torch.zeros(
                    (0, 4))
            elif key in ['gt_bboxes_3d']:
                empty_tensor = torch.zeros((0, inner.tensor.shape[-1]), dtype=inner.tensor.dtype,
                                           device=inner.tensor.device)
                empty_data = type(inner)(empty_tensor, box_dim=getattr(inner, 'box_dim', None))
            elif key in ['gt_bboxes', 'gt_labels', 'gt_labels_3d']:
                empty_data = torch.zeros((0,) + inner.shape[1:], dtype=inner.dtype,
                                         device=getattr(inner, 'device', 'cpu'))
            elif key == 'gt_masks':
                # Determine the appropriate empty mask representation based on the inner type
                empty_data = type(inner)([], inner.height, inner.width) if hasattr(inner, 'height') else []
            elif key == 'img_metas':
                meta_copy = copy.deepcopy(inner[0] if isinstance(inner, list) and inner else inner)
                if isinstance(meta_copy, dict):
                    meta_copy['is_prev_frame_valid'] = False
                empty_data = meta_copy
            elif isinstance(inner, torch.Tensor):
                empty_data = torch.zeros_like(inner)
            else:
                empty_data = copy.deepcopy(inner)

            # Preserve DataContainer structure if it existed in current data
            if isinstance(val, DataContainer):
                data_prev[key] = DataContainer(empty_data, stack=val.stack, cpu_only=val.cpu_only)
            else:
                data_prev[key] = empty_data
    ...
    def bbox2result_kitti2d(self,
        ...
        if pklfile_prefix is not None:
            ...
            if not pklfile_path.endswith(('.pkl', '.pickle')):
                pklfile_path += '.pkl'
