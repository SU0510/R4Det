...
class AnchorTrainMixin(object):
    ...
    def anchor_target_3d(self,
        ...
        (all_labels, all_label_weights, all_bbox_targets, all_bbox_weights,
         ...
         neg_inds_list) = multi_apply(
             ...
             gt_labels_list,
             input_metas,
             bev_semantic_mask_list if bev_semantic_mask_list is not None else [None] * num_imgs,
    ...
    def anchor_target_single_assigner(self,
        ...
        if len(gt_bboxes) > 0:
            ...
            ###############
            num_potential_pos = (assign_result.gt_inds > 0).sum().item()
            max_iou = assign_result.max_overlaps.max().item() if num_valid_anchors > 0 and len(gt_bboxes) > 0 else 0
            # print("\n" + ">>>" * 10 + " 对拍点 A (Assigner 输出) " + ">>>" * 10)
            # print(f"  - Assigner 找到的潜在正样本数量: {num_potential_pos}")
            # print(f"  - 所有 Anchors 的最大 IoU: {max_iou:.4f}")
            # print(">>>" * 30)
            ...
            # print("\n" + ">>>" * 10 + " 对拍点 B (Sampler 输出) " + ">>>" * 10)
            # print(f"  - Sampler 最终选出的正样本数量: {len(sampling_result.pos_inds)}")
            # print(f"  - Sampler 最终选出的负样本数量: {len(sampling_result.neg_inds)}")
            # print(">>>" * 30)
        ...
        if len(pos_inds) > 0:
            ...
            # =================== [对拍点 C: 方向目标编码] 开始 ===================
            # print("\n" + ">>>" * 10 + " 对拍点 C (Direction Target Encoding) " + ">>>" * 10)

            # 检查 get_direction_target 的输入
            # print("--- [输入] get_direction_target ---")
            # print(f"  - 输入 pos_bboxes 的 Sum: {sampling_result.pos_bboxes.sum():.4f}")
            # print(f"  - 输入 pos_bbox_targets 的 Sum: {pos_bbox_targets.sum():.4f}、diroffset{self.dir_offset}")
            ...
            pos_dir_targets = get_direction_target(
                ...
                one_hot=False)
            # 检查 get_direction_target 的输出
            # print("\n--- [输出] get_direction_target ---")
            # print(f"  - 输出 pos_dir_targets 的 Shape: {pos_dir_targets.shape}")
            # print(f"  - 输出 pos_dir_targets 的 Sum: {pos_dir_targets.sum().item()}")  # <--- 最关键指标
            # print(f"  - 输出 pos_dir_targets 的非零数量: {(pos_dir_targets != 0).sum().item()}")  # <--- 最关键指标
            # print(">>>" * 30 + "\n")
...
def get_direction_target(anchors,
    ...
    """
    # =================== [对拍点 1: 函数入口输入] 开始 ===================
    # print("\n" + "#" * 30 + " 进入 get_direction_target 函数 " + "#" * 30)
    # print("--- 正在对拍[函数入口]的原始输入 ---")

    # 详细打印输入 anchors 的统计信息
    # print("\n[输入] anchors (前 64 个正样本的 anchors):")
    # print(f"  - Shape: {anchors.shape}")
    # print(f"  - Sum:   {anchors.sum().item():.6f}")
    # print(f"  - Mean:  {anchors.mean().item():.6f}")
    # print(f"  - Std:   {anchors.std().item():.6f}")

    # 详细打印输入 reg_targets 的统计信息
    # print("\n[输入] reg_targets (bbox_coder 编码后的回归目标):")
    # print(f"  - Shape: {reg_targets.shape}")
    # print(f"  - Sum:   {reg_targets.sum().item():.6f}")
    # print(f"  - Mean:  {reg_targets.mean().item():.6f}")
    # print(f"  - Std:   {reg_targets.std().item():.6f}")
    # print("#" * 80)
    # =====================================================================
    # print(f"dir_offset：{dir_offset}、{dir_limit_offset}")

    rot_gt = reg_targets[..., 6] + anchors[..., 6]
    offset_rot = limit_period(rot_gt - dir_offset, 0, 2 * np.pi)    #原本是这样offset_rot = limit_period(rot_gt - dir_offset, dir_limit_offset, 2 * np.pi)
    # --- 最终对拍点 D: 检查 floor 的输入 ---
    value_before_floor = offset_rot / (2 * np.pi / num_bins)

    # print("\n" + ">>>" * 10 + " 对拍点 D (torch.floor 输入) " + ">>>" * 10)
    # print(f"--- [输入] torch.floor ---")
    # print(f"  - 计算出的原始方向 bin 值 (Shape): {value_before_floor.shape}")
    # print(f"  - 原始 bin 值的 Min: {value_before_floor.min().item():.6f}")
    # print(f"  - 原始 bin 值的 Max: {value_before_floor.max().item():.6f}")
    # print(f"  - 原始 bin 值的 Mean: {value_before_floor.mean().item():.6f}")
    # 筛选出那些非常接近整数边界的值，它们最可疑
    # (x - floor(x)) < 0.001 or (x - floor(x)) > 0.999
    diff = torch.abs(value_before_floor - torch.round(value_before_floor))
    suspicious_values = value_before_floor[diff < 0.001]
    # if len(suspicious_values) > 0:
        # print(f"  - 发现 {len(suspicious_values)} 个接近整数边界的可疑值:")
        # print(f"    {suspicious_values[:5]}")  # 最多打印5个
    # else:
        # print("  - 未发现接近整数边界的可疑值。")
